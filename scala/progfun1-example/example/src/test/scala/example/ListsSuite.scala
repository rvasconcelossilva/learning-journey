 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum of a few numbers - is not equal") {
    assert(sum(List(4, 5, 8)) === 3)
  }

  test("sum - list is empty") {
    intercept[IllegalArgumentException] {
      sum(List())
    }
  }

  test("sum - elements equal 0") {
    intercept[IllegalArgumentException] {
      sum(List(0, 0, 0))
    }
  }

  test("sum - there is negative elements") {
    intercept[IllegalArgumentException] {
      sum(List(-10, 15, 8))
    }
  }

  test("sum - there is duplicate elements") {
    intercept[IllegalArgumentException] {
      sum(List(-1, 0, 2))
    }
  }
    ...
    test("max - list is empty") {
      intercept[IllegalArgumentException] {
        max(List())
      }
    }

    test("max - elements equal 0") {
      intercept[IllegalArgumentException] {
        max(List(0, 0, 0))
      }
    }

    test("max - there is negative elements") {
      intercept[IllegalArgumentException] {
        max(List(-1, 3, 2))
      }
    }

    test("max - there is duplicated elements") {
      intercept[IllegalArgumentException] {
        max(List(2, 3, 2))
      }
    }
